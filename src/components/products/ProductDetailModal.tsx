...
export default function ProductDetailModal({
  ...
  const [localVariants, setLocalVariants] = useState<any[]>(Array.isArray(variants) ? variants : []);
  ...
  useEffect(() => {
    ...
    if (selectedVariant) {
      const found = localVariants.find((v) => String(v.id) === String(selectedVariant.id));
      if (found) {
  ...
  async function fetchVariants() {
    ...
    try {
      const res = await fetch(`/api/items?productId=${encodeURIComponent(String(product.id))}`, { cache: "no-store" });
      if (!res.ok) {
        // do not throw — keep current localVariants
        console.warn("ProductDetailModal: failed to fetch variants", res.status);
        return;
      }
  ...
  async function handleConfirmDelete() {
    ...
    setLocalVariants((prev) => prev.filter((x: any) => String(x.id) !== String(confirmTarget.id)));
  ...
  useEffect(() => {
    if (open) startAutoRotate(); else stopAutoRotate();
    ...
    <AnimatePresence>
      {open && (
        <motion.div className="fixed inset-0 z-50 flex items-center justify-center p-6" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
          ...
          <motion.div className="relative z-10 w-full max-w-5xl h-157 bg-white rounded-lg shadow-2xl overflow-auto" initial={{ y: -12, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: -12, opacity: 0 }} transition={{ duration: 0.16 }}>
            <div className="flex items-center gap-2 justify-end pr-4 pt-4">
              ...
              <button onClick={() => onAddItemForProduct?.(product)} className="px-3 py-2 rounded border text-sm">Add Variant</button>
              ...
              <div className="md:col-span-1 ">
                <div className="bg-gray-50 rounded overflow-hidden h-108 w-108 flex items-center justify-center">
                  <AnimatePresence mode="wait">
                    {mainImage ? (
                      <motion.img key={mainImage} src={mainImage} alt={selectedVariant?.title || product?.title || ""} initial={{ opacity: 0, x: 8, scale: 0.995 }} animate={{ opacity: 1, x: 0, scale: 1 }} exit={{ opacity: 0, x: -8, scale: 0.995 }} transition={{ duration: 0.45, ease: "easeInOut" }} className="object-cover w-full h-full" />
                    ) : (
                      <motion.div key="no-image" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="text-xs text-gray-400">No image</motion.div>
                    )}
                  </AnimatePresence>
                </div>
                ...
                <div className="mt-3 flex items-center gap-3 justify-center overflow-auto py-1">
                  {localVariants.length === 0 ? (
                    ...
                    localVariants.map((v: any, i: number) => {
                      ...
                      const active = selectedVariant && String(selectedVariant.id) === String(v.id);
                      return (
                        <motion.button key={v.id} onClick={() => pickVariant(v, i)} whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.98 }} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2, delay: i * 0.03 }} className={`flex-shrink-0 rounded border border-gray-300 cursor-pointer overflow-hidden ${active ? "ring-2 ring-[#2563EB] w-18 h-18" : "bg-white w-12 h-12"}`} aria-pressed={active}>
                          {thumb ? <motion.img src={thumb} alt={v.title} className="object-cover w-full h-full" initial={{ scale: 1 }} animate={active ? { scale: 1.03 } : { scale: 1 }} transition={{ duration: 0.25 }} /> : <div className="text-xs text-gray-400 p-2">No image</div>}
                        </motion.button>
                      );
                    })
                  )}
              ...
              <div className="md:col-span-2 flex flex-col">
                <div className="flex items-start gap-4">
                  <div className="flex-1">
                    ...
                    <div className="text-sm text-gray-500 mt-1">{product?.category ?? ""} {product?.code ? ` • ${product.code}` : ""} • {product?.type ?? product?.uom}</div>

                    <div className="mt-2">
                      <div className="text-xs text-gray-500">Item code</div>
                      <div className="text-sm font-medium">{(selectedVariant && (selectedVariant["item code"] ?? selectedVariant["item_code"] ?? selectedVariant.itemCode)) || "-"}</div>
                    </div>
                  </div>
                </div>

                <div className="mt-4 text-sm text-gray-700 whitespace-pre-line">{selectedVariant?.detail ?? product?.description ?? "-"}</div>
                ...
                <div className="mt-auto text-right text-xs text-gray-400">Variants: {localVariants.length}</div>
              </div>
            </div>
          </motion.div>

          <ConfirmDialog open={confirmOpen} title="Hapus variant" description={confirmTarget ? `Yakin ingin menghapus variant "${confirmTarget.title}"?` : ""} onConfirm={handleConfirmDelete} onCancel={() => setConfirmOpen(false)} confirmLabel="Yes, delete" cancelLabel="Cancel" />
        </motion.div>
      )}
    </AnimatePresence>
  );
